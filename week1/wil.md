...
const SecondaryButton = ({
  ...
  const handleClickSubmissionComplete = async ({
    ...
    if (response.success) {
      ...
      function closure({
        ...
        // setTimeout(() => {
        //   console.log(assignmentSubmissionStatus, assign, assignRef, "toast");
        //   if (assign === "SUCCESS") {
        //     toast.success("과제 제출이 완료되었습니다.");
        //   } else if (assign === "FAILURE") {
      ...
      closure({ assignmentSubmissionStatus: assign });
    }
  };
  ...
  console.log(assignmentSubmissionStatus, assign, assignRef, "secondartButton");
  return (
    <Button
      disabled={buttonsDisabled}
      ...
      onClick={() => {
        handleClickSubmissionComplete({ assignmentSubmissionStatus: assign });
      }}
    >
      {secondaryButtonText}
    </Button>
